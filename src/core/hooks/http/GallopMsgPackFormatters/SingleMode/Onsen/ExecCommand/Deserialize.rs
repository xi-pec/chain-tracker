...
type SingleModeOnsenExecCommandHook = unsafe extern "C" fn(*mut c_void, *mut c_void, *mut c_void, *mut c_void, *mut c_void) -> *mut c_void;

#[inline(never)]
#[no_mangle]
unsafe extern "C" fn single_mode_onsen_exec_command_hook(this: *mut c_void, bytes: *mut c_void, offset: *mut c_void, resolver: *mut c_void, size: *mut c_void) -> *mut c_void {
    let Some(plugin) = PLUGIN.get()
    else { return std::ptr::null_mut() };

    let Ok(trampoline) = plugin.il2cpp.get_trampoline(single_mode_onsen_exec_command_hook as Address)
    else { return std::ptr::null_mut() };

    let original: SingleModeOnsenExecCommandHook = std::mem::transmute(trampoline);
...
pub unsafe fn init(image: &str, namespace: &str, class: &str) {
    ...
    plugin.core.hooks.install(
        ...
        "Deserialize", 4,
        single_mode_onsen_exec_command_hook as Address
    );
}