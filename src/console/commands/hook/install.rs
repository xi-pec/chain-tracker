...
pub unsafe fn hook_install(args: &mut Split<'_, &str>) {
    ...
    let result = hooks.install(method, param_count);
    
    if result { log(format!("Successfully installed hook on method [{}] {}.{}::{}.", image_name, namespace, class_name, method_name))}
    else { log(format!("Could not install hook on method [{}] {}.{}::{}.", image_name, namespace, class_name, method_name)) }
}