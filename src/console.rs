...
static mut MY_PRIVATE_OUT: HANDLE = INVALID_HANDLE_VALUE;
...
pub unsafe fn init_console() {
    ...
    let mut private_mode: u32 = 0;
    if GetConsoleMode(MY_PRIVATE_OUT, &mut private_mode) != 0 {
        SetConsoleMode(MY_PRIVATE_OUT, private_mode | ENABLE_VIRTUAL_TERMINAL_PROCESSING);
    }
...
fn raw_write<S: AsRef<str>>(text: S) {
    unsafe {
        if MY_PRIVATE_OUT == INVALID_HANDLE_VALUE { return; }
        let mut written = 0;
        let s = text.as_ref();
        WriteFile(
            MY_PRIVATE_OUT,
